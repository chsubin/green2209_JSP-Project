				  ...
				  <div class="card-body">
				    <h4 class="card-title">Recent Reservation</h4>
							<table class="table table-bordered table-hover table-responsive-sm text-center">
								<c:if test="${empty rvVOS}"><tr><td class="text-center" colspan="3"><h6>최근 예약 내역이 없습니다.</h6></td></tr></c:if>
								<c:forEach var="rvVO" items="${rvVOS}">
									<tr class="text-center border">
										<td width="20%">
											<c:set var="fSNames" value="${fn:split(rvVO.fSName,'/')}"></c:set>
											<img src="${ctp}/data/room/${fSNames[fn:length(fSNames)-1]}" width="400px"/>
										<td>
										<td class="text-left"  style="width:30%">
											 예약번호 : <b>${rvVO.rvIdx}</b><br/><br/>
											 객실 이름 : ${rvVO.roomName}<br/><br/>
											 예약 일시 : ${fn:substring(rvVO.rvDay,0,19)}<br/><br/>
											 예약 날짜 : ${rvVO.checkIn}~${rvVO.checkOut}<br/><br/>
											 가격 : <fmt:formatNumber value="${rvVO.price}" pattern="#,##0"/>원<br/>
										 </td>
										 <td class="text-left" style="width:30%">
										 		<br/><br/><br/>
										 	 	<div class="badge badge-primary">checkIn</div> <font color="blue">${rvVO.rcheckIn}<c:if test="${empty rvVO.rcheckIn}">미입실</c:if></font><br/>
										 	 	<div class="badge badge-danger">checkOut</div> <font color="red">${rvVO.rcheckOut}<c:if test="${empty rvVO.rcheckIn}">미퇴실</c:if></font><br/>
										 </td>
									</tr>
								</c:forEach>
